...
<div class="relative" x-data="{ open: false, adjustWidth: 0 }" x-init="$watch('open', value => {
    ...
    <button
        class="flex flex-row items-center px-1 py-1 text-sm font-semibold whitespace-nowrap text-base hover:text-base/80"
        x-on:click="open = !open">
        
        {{ $trigger }} @if($showArrow) <x-ri-arrow-down-s-line x-bind:class="{ '-rotate-180' : open }"
                class="md:block hidden size-4 text-base ease-out duration-300" />
        @endif
        </button>

    <div x-ref="dropdown"
        class="absolute mt-4 px-1 py-1 bg-background-secondary/80 border border-neutral/50 shadow-xl rounded-md z-10 {{ $width ?? "w-48" }}"
        x-bind:style="{
            left: `-${adjustWidth}px`,
        }"
        x-show="open"
