...
<fieldset class="flex flex-col w-full {{ $divClass ?? '' }}">
    ...
    <select id="{{ $id ?? $name }}" {{ $multiple ? 'multiple' : '' }} {{ $attributes->except(['options', 'id', 'name', 'multiple', 'class']) }}
        class="block px-2.5 py-2.5 w-full text-sm text-base bg-background border-2 rounded-xl shadow-sm outline-none backdrop-blur-sm border-neutral/20 focus:border-primary/70 focus:ring-2 focus:ring-primary/30
        focus:outline-none transition-all duration-300 ease-in-out form-select disabled:bg-background-secondary/50 disabled:cursor-not-allowed ">
