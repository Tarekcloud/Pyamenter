<div class="mt-2">
    <div class="p-6 rounded-lg">
        ...
        <div class="text-center">
            <p class="mb-2">请使用微信扫描上方二维码完成支付</p>
        </div>
    </div>
</div>
...
<script>
    (function () {
        ...
        async function generateQR() {
            ...
            const svg = encodeQR('{{ $code_url }}', 'svg', {
                ecc: 'high',
                scale: 8,
                border: 2
            });
        ...
        function checkPayment() {
            window.paymentCheckInterval = setInterval(async () => {
                try {
                    const response = await fetch("{{ route('extensions.gateways.wechatpay.status', ['invoice' => $invoice->id]) }}", {
                        headers: {
